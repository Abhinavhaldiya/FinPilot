...
import LanguageSwitcherPage from "../LanguageSwitcher.tsx";
...
const DashboardLayout = () => {
  ...
  const getNavItems = (): NavItem[] => {
    const baseItems = [
      ...
      { titleKey: "nav.myClaims", url: "/claims", icon: Receipt, badge: 3 },
    ];
    ...
    baseItems.push({ titleKey: "nav.settings", url: "/settings", icon: Settings });
    return baseItems;
  };
        ...
        {isSidebarOpen && (
          <div className="p-4 border-b border-border">
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-muted">
              ...
              <span className="text-sm">{t("roleSelector")}: {currentRole.name}</span>
            </div>
          </div>
        )}
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-lg font-semibold">
                {t("goodMorning", { name: "Alex" })}
              </h2>
              <p className="text-sm text-muted-foreground">
                {t("dashboardRole", { role: currentRole.name })}
              </p>
            </div>
          </div>
